...
- Kullanıcıya görünen tüm uygulama metni next-intl üzerinden gelir. Yeni key'i 8 dosyaya (`en`, `tr`, `hi`, `es`, `fr`, `de`, `zh`, `ru`) ekle; 39 namespace vardır (`LandingNext` şimdilik yalnız `en`/`tr`de — diğer diller İngilizceye düşer).
...
- Content write/delete yollarında `syncPageLinks`, `removeOutgoingPageLinks` ve tombstone yan etkilerini koru; `seed.ts` doğrudan DB yazdığı için ayrıca kontrol edilmelidir. Çöp kutusuna silme başka sayfanın içeriğini değiştirmez: silinen öğeye giden linkler kalır (editör soluk gösterir) ve `purgeReferencesTo` + `removePageLinksFor` yalnızca çöp kopyası kalıcı olarak gidince (`releaseTrashedReferences`) ya da çöp kopyası yazmayan bir silmede çalışır. Ayrıntı: `AGENTS.md` → **Trash & Version History**.
...
- Ajan token'ı sahibinin üyeliğini aşamaz: MCP her istekte token sahibinin üyeliğini ve rolünü yeniden okur (`src/lib/services/agentAccess.ts`). Üye değilse 401 döner, viewer'a `read` verilir. Üye çıkaran ya da rolü viewer yapan yeni bir yol `revokeAgentAccess` / `restrictAgentAccessToRead` çağırmalıdır. Ayrıntı: `AGENTS.md` → **Project Install** §1.
