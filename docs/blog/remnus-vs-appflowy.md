...
| Native MCP server (agent connects in) | No, an open feature request (GitHub issue #8043, opened June 2025) remains unresolved | Yes, first-party, 25 tools |
...
Remnus was built the opposite way: the MCP server is the first-party interface, live from the start, with 25 tools split 11 read and 14 write, plus 6 resource templates and 7 prompt templates, maintained by the same team that builds the product.
