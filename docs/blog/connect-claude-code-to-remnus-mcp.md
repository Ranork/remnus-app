...
**3. Check the tool count** inside a session with `/mcp`. The panel shows how many tools each server exposes. Remnus publishes **27**: 11 read tools and 16 write tools.
...
**No tools appear.** Confirm the scope you connected at — read tokens still expose all 11 read tools, so an empty list means a connection problem, not a scope problem. Check `claude mcp get remnus`.
