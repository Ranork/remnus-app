...
- **A full primitive surface.** The server exposes **24 tools** (11 read, 13 write), **6 resources**, and **7 prompts**. Read tools include token-budgeted `prepare_context`, search, listing, page/schema reads, batch page reads, graph traversal, and filtered database queries; write tools cover creating and updating pages and rows, batch updates and deletes, moving or bulk-moving items, and managing database schemas and views. Resources attach workspace structure, Knowledge Health, a page, a digest, or recent activity as cheap context. Prompts package recurring jobs — including `weekly-status-report`, `kanban-triage`, and `save-memory` / `recall-context`.
