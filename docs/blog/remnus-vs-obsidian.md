...
| Native MCP server (agent connects in) | No official core plugin; a forum request for one exists with no team commitment | Yes, first-party, 27 tools |
...
Remnus didn't leave that gap open. The MCP server ships with the product, built and maintained by the same team, covering 27 tools (11 read, 16 write) plus 7 resources and 7 prompt templates against the workspace's real data model, not a file-system shim.
