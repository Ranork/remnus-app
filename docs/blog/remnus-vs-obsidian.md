...
| Native MCP server (agent connects in) | No official core plugin; a forum request for one exists with no team commitment | Yes, first-party, 21 tools |
...
Remnus didn't leave that gap open. The MCP server ships with the product, built and maintained by the same team, covering 21 tools (11 read, 10 write) plus 6 resource templates and 7 prompt templates against the workspace's real data model, not a file-system shim.
...
Remnus prices the workspace itself, flat per owner rather than per seat: Free at $0, Startup at $10 a month, Professional at $29 a month, Enterprise custom. The entire 21-tool MCP surface works the same on every tier, Free included, because agent access is the product, not an add-on bolted beside it.
