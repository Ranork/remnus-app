...
| MCP tools exposed | 19, across search, content, database and view management, comments, and workspace admin | 27, split 11 read and 16 write, plus 7 resources and 7 prompt templates |
...
Remnus prices flat per workspace owner rather than per seat: Free at $0, Startup at $10 a month, and Professional at $29 a month, each covering a fixed seat and agent allowance rather than charging per head. The full 24-tool MCP surface, both read and write, works identically on every tier, Free included. The only things that scale with plan are seats, connected agents, storage, and audit log retention window, not which MCP tools are available.
...
Remnus exposes 27 tools. Read: `prepare_context`, `search_workspace`, `list_workspace`, `get_page`, `get_pages`, `get_database_schema`, `query_database`, `list_members`, `query_audit_log`, `get_changes_since`, `get_related_pages`. Write: `create_page`, `bulk_create_pages`, `update_page`, `bulk_update_pages`, `delete_page`, `bulk_delete_pages`, `move_item`, `bulk_move_items`, `create_database`, `update_database_schema`, `create_database_view`, `update_database_view`, `delete_database_view`, `create_dashboard`, `update_dashboard`, `add_comment`. `prepare_context` builds a trust/freshness-aware product context pack within an explicit token budget. A few details worth calling out because they show up in real agent workflows: `update_page` and `bulk_update_pages` merge properties instead of overwriting them, so an agent updating one field never clobbers the rest of a row. `delete_page` previews what it would delete unless you explicitly pass `confirm: true`, and `bulk_delete_pages` does the same for up to 100 items at once. `update_database_schema` protects the title column and requires explicit confirmation to remove any other column. A read-scoped token calling any write tool gets an explicit error instead of silently failing.
