...
- New migrations: `when` value must be greater than all existing — next migration `when > 1781900000000` (0031_signup_attribution = user signup_* cols; 0032_demo_feedback = demo_feedback table; 0033_mailing = email_campaigns + email_log + user email_unsubscribed_at/email_suppressed; 0034_agent_usage = agent_activity REBUILD: token_id nullable + oauth_token_id + owner_user_id + response_bytes — fixes silent OAuth audit drop, powers usage meters; 0035_deleted_items = deleted_items tombstone table, powers get_changes_since; 0036_page_links = page_links content-derived link graph, powers get_related_pages — apply script + backfill-page-links.ts, both run on local + Turso; 0037_user_sessions_platform = nullable user_sessions.platform column (web|tauri), from a concurrent origin commit; 0038_account_deletion_tokens = DB-backed single-use confirm token for GDPR account deletion, powers requestAccountDeletion/confirmAccountDeletion)
...
- Many recent migrations (0017–0037) are NOT in `_journal.json` — applied manually via `src/db/apply-00xx-*.ts` scripts (libsql `batch()` silently no-ops DDL). Apply each to BOTH local (`DATABASE_URL="file:local.db" npx tsx ...`) and Turso (plain `npx tsx ...` reads `.env`). **`.env`'s Turso (`remna-db-ranork`) IS production remnus.com** — plain runs are prod operations.
